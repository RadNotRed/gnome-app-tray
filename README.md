...
- Right-clicks open a separate application menu next to the icon, keeping the tray compact. Submenus expand on click, and long labels are shortened with an ellipsis. Escape dismisses the app menu first, then the tray; clicking outside both or choosing an action closes both. Middle-click and scroll actions are forwarded when supported.
