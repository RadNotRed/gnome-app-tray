...
function getIconGeometry(indicator, sourceIcon, iconSize) {
  try {
    const pixmaps = indicator?.icon?.pixmap;
    if (!pixmaps || typeof pixmaps.n_children !== 'function') {
      const source = getSourceDimensions(sourceIcon);
      return { ...fitIconDimensions(iconSize, source.width, source.height), scale: 1 };
    }

    let bestPixmap = null;
    for (let index = 0; index < pixmaps.n_children(); index++) {
      const pixmap = pixmaps.get_child_value(index);
      const width = pixmap.get_child_value(0).unpack();
      const height = pixmap.get_child_value(1).unpack();
      if (width <= 0 || height <= 0) continue;
      if (!bestPixmap || width * height > bestPixmap.width * bestPixmap.height)
        bestPixmap = { pixmap, width, height };
    }
    ...
    const { pixmap, width, height } = bestPixmap;
    const dimensions = fitIconDimensions(iconSize, width, height);
    const pixels = pixmap.get_child_value(2).deep_unpack();
    if (!pixels || pixels.length < width * height * 4) return { ...dimensions, scale: 1 };
    ...
    for (let offset = 0; offset < width * height * 4; offset += 4) {
      if (pixels[offset] <= PIXMAP_ALPHA_THRESHOLD) continue;
    ...
    if (right < left || bottom < top) return { ...dimensions, scale: 1 };
    ...
    return {
      ...dimensions,
      scale: scale < 1.05 ? 1 : clamp(scale, 1, MAX_ICON_SCALE),
    };
  } catch (_error) {
    const source = getSourceDimensions(sourceIcon);
    return { ...fitIconDimensions(iconSize, source.width, source.height), scale: 1 };
...
const TrayIconButton = GObject.registerClass(
  ...
  class TrayIconButton extends St.Button {
    ...
    sync(info, iconSize) {
      ...
      const geometry = getIconGeometry(info.indicator, info.sourceIcon, iconSize);
...
export const AppTrayMenu = GObject.registerClass(
  ...
  class AppTrayMenu extends PanelMenu.Button {
    ...
    _openContextMenuSession(entry) {
      const context = this._findContextMenuClient(entry.item.menu);
      if (!context) return;
      ...
      const addMenu = (menu, depth = 0) => {
        ...
        for (const sourceItem of this._menuItems(menu)) {
          ...
          if (submenu) {
            ...
            if (submenuItems.length > 0) addMenu(submenu, depth + 1);
            continue;
          }
