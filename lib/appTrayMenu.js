...
const DEFAULT_FOOTER_TEXT = 'Drag icons to rearrange temporarily';
  ...
  class TrayIconButton extends St.Button {
    constructor(info, iconSize, callbacks) {
      ...
      this.connect('notify::hover', () => {
        this._callbacks.hover(this.hover ? this.accessible_name : null);
      });
      ...
      this._draggable.connect('drag-begin', () => {
        ...
        this._callbacks.dragBegin(this.panelId);
      });
    ...
    sync(info, iconSize) {
      ...
      this._setIcon(info.sourceIcon, info.iconName);
    }

    _setIcon(sourceIcon, fallbackIconName) {
      let gicon = null;
      ...
      if (
        ...
        if (this._iconBin.child instanceof St.Icon) this._iconBin.child.icon_size = this._iconSize;
        return;
      }
...
export const AppTrayMenu = GObject.registerClass(
  class AppTrayMenu extends PanelMenu.Button {
    constructor(settings, openPreferences) {
      ...
      this._activeIndicatorMenu = null;
    ...
    _buildMenu() {
      ...
      this._footerLabel = new St.Label({
        ...
        style_class: 'gnome-app-tray-footer',
        x_align: Clutter.ActorAlign.CENTER,
      });
    ...
    _addEntry(info) {
      const callbacks = {
        ...
        hover: (title) => this._setFooter(title),
        dragBegin: (panelId) => this._onDragBegin(panelId),
    ...
    _removeEntry(panelId) {
      ...
      if (this._activeIndicatorMenu?.entry === entry)
        this._restoreIndicatorMenu(this._activeIndicatorMenu, true);
    ...
    _rebuildGrid() {
      ...
      this._footerLabel.visible = entries.length > 0;
    ...
    _setFooter(title) {
      if (this._destroyed || !this._footerLabel) return;
      this._footerLabel.text = title || DEFAULT_FOOTER_TEXT;
    }
    ...
    _openContextMenu(panelId, eventInfo) {
      ...
      this.menu.close();
      if (this._activeIndicatorMenu) this._restoreIndicatorMenu(this._activeIndicatorMenu, true);

      const state = {
        entry,
        menu,
        originalSourceActor: menu.sourceActor,
        originalFocusActor: menu.focusActor,
        openStateId: 0,
        destroyId: 0,
      };
      this._activeIndicatorMenu = state;

      try {
        // Anchor to the persistent top-bar chevron. A popup cell becomes
        // unmapped as soon as the overflow menu closes.
        menu.sourceActor = this;
        menu.focusActor = this;
        state.openStateId = menu.connect('open-state-changed', (_menu, open) => {
          if (!open) this._restoreIndicatorMenu(state, false);
        });
        state.destroyId = menu.connect('destroy', () => {
          if (this._activeIndicatorMenu === state) this._activeIndicatorMenu = null;
        });
      } catch (error) {
        logError(error, `[AppTray] Failed to prepare menu for ${entry.info.title}`);
        this._restoreIndicatorMenu(state, false);
        return;
      }

      this._scheduleAction(() => {
        if (this._activeIndicatorMenu !== state || entry.dead) return;
        menu.open();
        if (!menu.isOpen) this._restoreIndicatorMenu(state, false);
      });
    }

    _restoreIndicatorMenu(state, closeMenu) {
      if (!state || this._activeIndicatorMenu !== state) return;
      this._activeIndicatorMenu = null;

      try {
        if (closeMenu && state.menu.isOpen) state.menu.close();
      } catch (_error) {
        // The foreign menu may already be disposed.
      }

      for (const id of [state.openStateId, state.destroyId]) {
        if (!id) continue;
        try {
          state.menu.disconnect(id);
        } catch (_error) {
          // The menu may have destroyed itself.
        }
      }

      try {
        state.menu.sourceActor = state.originalSourceActor;
        state.menu.focusActor = state.originalFocusActor;
      } catch (_error) {
        // The menu may have destroyed itself.
      }
    }
    ...
    _onDragBegin() {
      this._setFooter('Release over another icon to move it');
    }
