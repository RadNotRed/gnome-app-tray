...
function getPixmapScale(indicator) {
  try {
    const pixmaps = indicator?.icon?.pixmap;
    if (!pixmaps || typeof pixmaps.n_children !== 'function') return 1;
    ...
    return scale < 1.05 ? 1 : clamp(scale, 1, MAX_ICON_SCALE);
  } catch (_error) {
    return 1;
  }
}
...
const TrayIconButton = GObject.registerClass(
  ...
  class TrayIconButton extends St.Button {
    ...
    sync(info, iconSize) {
      ...
      const scale = getPixmapScale(info.indicator);
    ...
    _setIcon(sourceIcon, fallbackIconName, scale) {
      ...
      const renderSize = this._iconSize;
